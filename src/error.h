...
#define VERBOSE(msg, ...) { \
    if (sigtk_log_level >= LOG_VERB) { \
        fprintf(stderr, VERBOSE_PREFIX msg "\n", __func__, __VA_ARGS__); \
    } \
}

#define INFO(msg, ...) { \
    if (sigtk_log_level >= LOG_INFO) { \
        fprintf(stderr, INFO_PREFIX msg NO_COLOUR "\n", __func__, __VA_ARGS__); \
    } \
}

#define WARNING(msg, ...) { \
    if (sigtk_log_level >= LOG_WARN) { \
